...
class NativeDownloadService {
  ...
  Future<Map<String, dynamic>> _downloadByAwemeId(
    ...
    try {
      ...
      final authorDir = author.isNotEmpty ? '$savePath/$safeAuthor' : savePath;
  ...
  Future<Map<String, dynamic>> downloadDouyinImages(
    ...
    for (var i = 0; i < total; i++) {
      ...
      final filename = '${title}_${i + 1}$primaryExt';
      final filePath = await _downloadOneImage(
        url: primaryUrl,
        savePath: savePath,
        filename: filename,
        index: i,
      ...
      if (filePath != null) {
        ...
        // 通知 bridge_base：已下载字节数（精确累计）
        if (onProgress != null) onProgress(totalBytes, totalBytes);
      }
    }
    final msg = videoCount > 0
        ? '已保存 $count 项 (含 $videoCount 个动图)，共 $totalBytes 字节'
        : '已保存 $count/${images.length} 张图片，共 $totalBytes 字节';
    if (count == 0) {
      return {'success': false, 'message': '图集下载失败'};
    }
    return {
      'success': true,
      'title': title,
  ...
  /// 下载单张图（带超时与重试），返回文件绝对路径
  Future<String?> _downloadOneImage({
    required String url,
    ...
    try {
      return await downloadFile(
        ...
        savePath,
        filename,
        onProgress: (downloaded, bytesTotal) {
          if (onProgress == null) return;
          // 单张进度透传：已下载字节 / 总字节
          onProgress(downloaded, bytesTotal > 0 ? bytesTotal : downloaded);
        },
      ).timeout(const Duration(seconds: 60), onTimeout: () {
        debugPrint('downloadDouyinImages: 第${index + 1}张 60s 超时');
        onStatus?.call('⚠️ 第${index + 1}张超时，跳过');
        return null;
      });
    } catch (e) {
      debugPrint('downloadDouyinImages: 第${index + 1}张异常: $e');
      onStatus?.call('⚠️ 第${index + 1}张失败: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>> _processXhsNote(
    ...
    if (imageList.isNotEmpty) {
      ...
      for (var i = 0; i < total; i++) {
        ...
        final fp = await _downloadOneImage(
          url: imgUrl,
          savePath: authorDir,
          filename: '${fileBaseName}_${i + 1}.jpg',
          index: i,
          total: total,
          onProgress: onProgress,
          onStatus: onStatus,
        ...
        if (fp != null) {
          count++;
          try {
            totalBytes += await File(fp).length();
          } catch (_) {}
          if (onProgress != null) onProgress(totalBytes, totalBytes);
        }
      }
      final msg = '已保存 $count/$total 张图片，共 $totalBytes 字节';
      return count > 0
          ? {
              'success': true,
              'title': title,
